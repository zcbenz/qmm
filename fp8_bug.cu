...
/// Partial specialization for Array<float, N> <= Array<float_e4m3_t, N>
template <int N, FloatRoundStyle Round>
struct NumericArrayConverter<float, float_e4m3_t, N, Round> {
  using result_type = Array<float, N>;
  using source_type = Array<float_e4m3_t, N>;
...
#if 1
      result[i] =
          reinterpret_cast<const __nv_fp8_e4m3*>(&source[i])->operator float();
#else
...
int main(int argc, char** argv) {
  ...
  using Element = float;
  using Quant = cutlass::float_e4m3_t;
